...
def fetch_data_from_mysql(medicine_id):
    ...
    WHERE pti.medicine = %s
    ORDER BY pti.created_at ASC
    """
    
    # Use Pandas to fetch data into a DataFrame
    df = pd.read_sql(query, connection, params=(medicine_id,))
    connection.close()
...
if __name__ == "__main__":
    try:
        # Get medicineId from command-line arguments
        if len(sys.argv) < 2:
            raise ValueError("Medicine ID not provided. Pass it as the first argument.")
        
        medicineId = sys.argv[1]

        # Fetch data from MySQL
        data = fetch_data_from_mysql(medicineId)
